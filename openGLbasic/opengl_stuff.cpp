...
namespace ns_opengl 
{
	...
	void Basic_OpenGLApp::createWindow( int argc, char *argv[], std::string p_Title )
	{
		...
		*/
		...
		m_WinID = glutCreateWindow( "Crowd Simulation. By Yu-An Chang" );
		...
		GLUI_Master.set_glutIdleFunc( gb_idle );
...
void ogl_drawSimpleCircleXZ( float x, float y, float z, float r, int numSegments )
{
	...
	{
		float a = i/(float)numPoints*3.14*2.0;
...
void ogl_drawUnFilledRectXZ( float minx, float maxx, float minz, float maxz, float y )
{
	glBegin( GL_LINE_LOOP );
	glNormal3f(0.0, 1.0, 0.0);
	glVertex3f( minx, y, minz );
