...
public:
	...
	void setGridSpace(float minx, float maxx, float minz, float maxz)
	{
		this->minx = minx;
		this->maxx = maxx;
		this->minz = minz;
	...
	void update( float time_step = 0.0 ) {
		...
		if ( time_step <= 0.0 ) time_step = 0.01;
		max_counter = memory_time / time_step;
		for (int i = 0; i < nx*nz; ++i ) {
			//counter[i] = 0;
			--counter[i];
			if ( counter[i] < 0 ) counter[i] = 0;
			else if ( counter[i] > max_counter ) counter[i] = max_counter;
		}
	}
